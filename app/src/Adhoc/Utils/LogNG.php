 ...
 * Gestion des logs
 *
 * @author Guillaume Seznec <gseznec@seznec.fr>
 */
class LogNG
{
    ...
    protected static function getOutput(): mixed
    {
        if (php_sapi_name() === 'cli') {
            ...
            if (defined('LOG_PATH')) {
                return fopen(LOG_PATH . '/' . self::getLogFile(), 'a');
            }
            return fopen(dirname(dirname(dirname(__DIR__))) . '/../log/' . self::getLogFile(), 'a');
        }
    }

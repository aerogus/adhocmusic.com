...
{include file="common/boxstart.tpl" boxtitle="Forums"}

<table id="forums" style="width: 100%">
  <thead>
    <tr>
      <th>Forum</th>
  ...
  {foreach from=$forums key=cpt item=forum}
    <tr class="{if $cpt is odd}odd{else}even{/if}">
      <td><a href="/forums/forum/{$forum.id_forum|escape}"><strong>{$forum.title|escape}</strong></a><br />{$forum.description|escape}</td>

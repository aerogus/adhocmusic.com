    ...
    if(id_country == 'FR') {
      ...
      $.getJSON('/geo/getcity.json', { d:id_departement }, function(data) {
        ...
        $.each(data, function(city_id, city_name) {
          if(membre_id_city == city_id) { selected = ' selected="selected"'; } else { selected = ''; }
      ...
      *}
      ...
      <li>
        ...
        <select id="id_country" name="id_country" style="float: right;">
          ...
          <option value="{$country_id}"{if $membre->getIdCountry() == $country_id} selected="selected"{/if}>{$country_name.fr|escape}</option>
          {/foreach}
        </select>
